        ...
        if(err){
            ...
            async.waterfall([ function(callback){callback(null, userid)}, // waterfall에 userid를 넘겨주기 위한 함수
                ...
                if (err) {
                    ...
                    connection.release();
                    logger.debug("createCoupleCode 중복되는 코드가 있습니다.");
                    sendAPIResult(res, err, null, "createCoupleCode 중복되는 코드가 있습니다. 다시 시도해주세요.");
                } else if(result == 'user_exist'){
                    connection.release();
                    logger.debug("createCoupleCode 이미 코드가 있는 사용자입니다.");
                    sendAPIResult(res, err, null, "createCoupleCode 이미 코드가 있는 사용자입니다.");
                } else {
                    ...
                    connection.query(insertQuery, [userid, authkey], function(err, result){
                        if(err){
                            ...
                            connection.query(selectQuery, [userid], function(err, rows, fields){
                                if (err) {
                                    connection.release();
                                    logger.debug("createCoupleCode 커플인증코드 삭제 스케쥴 설정 중 에러(1) : ", err);
                                    sendAPIResult(res, err, null, "createCoupleCode 커플인증코드 삭제 스케쥴 설정 중 에러(1) : ");
                                } else {

                                    // 시간 (년, 월(0-11), 일, 시(0-23), 분, 초)
                                    var scheduledTime = new Date(rows[0].year, rows[0].month, rows[0].day, rows[0].hour, rows[0].minute, rows[0].second);
                                    var scheduledJob = schedule.scheduleJob(scheduledTime, function(){
                                        // 스케쥴용 커넥션 연결
                                        connectionPool.getConnection(function(err, connection){
                                            if(err){
                                                logger.debug("createCoupleCode-schedule ConnectionPool 에러 발생 : "+err);
                                                sendAPIResult(res, err, null, "createCoupleCode-schedule ConnectionPool 에러 발생 : ");
                                            } else {
                                                // delete 쿼리
                                                ...
                                                connection.query(deleteQuery, [userid], function(err, results){
                                                    if (err) {
                                                        connection.release();
                                                        logger.debug('커플인증코드 10분후 삭제 오류. 사용자 : '+ userid);
                                                    } else {
                                                        if(results.affectedRows){	// 삭제된 레코드가 있는 경우 로그 기록
                                                            logger.debug('커플코드 삭제됨. 사용자 : '+userid);
                                                        }
                                                        connection.release();
                                                    }
                                                });
                    ...
                    if(error){	// 도중에 에러가 발생해서 error 값이 null이 아닌경우 실패값 전송
                        connection.release();
                        logger.debug("createCoupleCode 커플 인증코드 생성 에러 발생 : "+error);
                        sendAPIResult(res, error, null, "createCoupleCode 커플 인증코드 생성 에러 발생 : ");
                    } else {	// error값이 null인 상태로 무사히 실행된 경우
                        connection.release();
                        sendAPIResult(res, null, {}, "커플 인증코드 생성 신청 완료");
                    }
                }
            });
        }
...
function getCoupleCode(req, res) {
    connectionPool.getConnection(function(err, connection){
        if(err){
            logger.debug("getCoupleCode ConnectionPool 에러 발생 : "+err);
            sendAPIResult(res, err, null, "getCoupleCode ConnectionPool 에러 발생 : ");
        } else {
//			var userid = req.params.userid;	// url 파라미터로 받은 userid(사용자 식별자) 값
            ...
            connection.query(selectQuery, [userid], function(err, rows, fields){
                if(err){	// 쿼리문 에러 발생시
                    connection.release();
                    logger.debug("getCoupleCode 인증코드 받아오기 에러 발생 : "+err);
                    sendAPIResult(res, err, null, "getCoupleCode 인증코드 받아오기 에러 발생 : ");
                }
                ...
                if(rows.length){	// 검색된 값이 있는 경우;
                    ...
                    logger.debug("getCoupleCode 해당 사용자에 대한 인증코드가 없습니다.");
                    sendAPIResult(res, ' ', null, "해당 사용자에 대한 인증코드가 없습니다.");
                }
            });
        }
    });
...
function coupleSelect(userid, authcode, callback){
    connectionPool.getConnection(function(err, connection){
        if(err){
            logger.debug("coupleSelect ConnectionPool 에러 발생 : "+err);
            sendAPIResult(res, err, null, "coupleSelect ConnectionPool 에러 발생 : ");
        } else {
...
function coupleInsert(userid, other_userid, authcode, callback){
    connectionPool.getConnection(function(err, connection){
        if(err){
            logger.debug("coupleInsert ConnectionPool 에러 발생 : "+err);
            sendAPIResult(res, err, null, "coupleInsert ConnectionPool 에러 발생 : ");
        } else {
...
function authcodeDelete(authcode, callback){
    connectionPool.getConnection(function(err, connection){
        if(err){
            logger.debug("authcodeDelete ConnectionPool 에러 발생 : "+err);
            sendAPIResult(res, err, null, "authcodeDelete ConnectionPool 에러 발생 : ");
        } else {
        ...
        async.waterfall([ function(callback){
            ...
            function(err, result) {
                ...
                if(result == "no_code"){
                    ...
                    logger.debug("입력한 인증코드와 일치하는 정보가 없습니다. : ", err);
                    sendAPIResult(res, err, null, "입력한 인증코드와 일치하는 정보가 없습니다. 상대방의 인증코드를 다시 확인해주세요. : ");
                }else if(result == "db_error"){
                    // DB 쿼리문 실행중 오류가 발생한 경우
                    logger.debug("데이터베이스 작업 중 오류가 발생했습니다. : ",err);
                    sendAPIResult(res, err, null, "데이터베이스 작업 중 오류가 발생했습니다. : ");
                } else {
