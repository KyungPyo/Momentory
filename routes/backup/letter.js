						...
						if (err) {	// 쿼리문 처리중 오류 발생
							...
							function(err, results){
								...
								if(count <= 0){	// 검색된 편지 갯수만큼 반복이 끝났을 경우
									connection.release();
									// 성공 결과 전송
									sendAPIResult(res, null, results, "편지 목록 요청 성공");
								}
							});
						}
					});
			...
			connection.query(selectQuery, [letterid], function(err, rows, fields){
				if(err){
					res.json({
						"isSuccess" : false,
						"result" : null,
						"msg" : "DB에서 해당 편지의 첨부사진을 받아오는 과정에서 에러 발생"
					});
				} else {
					if(rows.length){
						var picture = [];
						...
						res.json({
							...
							"msg" : "편지에 첨부된 사진이 없거나 해당 편지가 없습니다."
						});
					}
				}
			});
			...
			connection.beginTransaction(function(err) {	// 트랜젝션 사용
				if (err) {
					...
					res.json({
						"isSuccess" : false,
						"result" : null,
						"msg" : "writeLetter Transaction 에러 발생 : "+err
					});
				} else {
					async.waterfall([
						...
						} else if (err && !(files)){	// 업로드한 파일이 없는데 에러 발생
							connection.rollback(function() {	// 작업 실패로 롤백
								connection.release();
								res.json({
									"isSuccess" : false,
